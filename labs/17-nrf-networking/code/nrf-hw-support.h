...
/**********************************************************
 * simple utility routines that use SPI to read/write 
 * registers on the NRF24L01+.
 */

// initialize SPI driver and the GPIO pins: return the spi_t 
// structure.  
...
/***************************************************************
 * Hardware accessors for each different NRF register.
 * These are right from the datasheet.
 *
 * NOTE: these are not all the ones you'd want.
 */
...
static inline int nrf_is_pwrup(nrf_t *nic) {
    return bit_isset(nrf_get8(nic, NRF_CONFIG), PWR_UP_BIT);
}
// power down mode.
static inline void nrf_set_pwrup_off(nrf_t *nic) {
    nrf_bit_clr(nic, NRF_CONFIG, PWR_UP_BIT);
    assert(!nrf_is_pwrup(nic));
}
// power up mode.
static inline void nrf_set_pwrup_on(nrf_t *nic) {
    nrf_bit_set(nic, NRF_CONFIG, PWR_UP_BIT);
    assert(nrf_is_pwrup(nic));
}
...
// extract pipeid from status value.
...
/****************************************************
 * NRF state: should probably extend.
 */
...
static inline int nrf_is_pwrdown(nrf_t *nic) {
    return !nrf_is_pwrup(nic);
}
